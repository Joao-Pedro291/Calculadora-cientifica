...
def trig(func):
    ...
    try:
        ...
        if shift_ativo:  # funções inversas
            if func == "sin":
...
def ponto_duplo():
    """Converte decimal ↔ DMS (graus, minutos, segundos)"""
    try:
...
def abc():
    try:
        txt = caixaCalc.get().strip()
        # Se tiver espaço e barra → converter de a b/c para decimal
        ...
        # Caso seja decimal → converter para a b/c
        else:
            valor = float(txt)
            inteiro = int(valor)
            resto = valor - inteiro
...
def criar_botao(frame,texto,linha,coluna,cor="lightgray",fg="black",largura=6,altura=2,colspan=1,rowspan=1,shiftable=False):
    botao = tk.Button(
        frame,text=texto,font=("Arial",12,"bold"),fg=fg,bg=cor,width=largura,height=altura,
        relief="raised",bd=2,command=lambda t=texto: processar_botao(t)
    )
